...
class IcoBazaar(ScraperBase):
    ...
    def scrape_listings(self, url):
        ...
        driver.find_element_by_xpath('/html/body/div[1]/div[2]/div/main/section/div[1]/div[2]/ul/li[5]/label/div').click()
        ...
        for i in range(len(elements)):
            urls.append(elements[i].get_property('href'))
    ...
    def scrape_profile(self, url):
        ...
        try:
            for a in bs.find_all('div', {'class': 'com-sidebar__info-line'}):
                try:
                    ...
                    self.logger.error('Key {} does not exist in sidebar'.format(re.sub(':', '', a.find('span').text.strip())))
                    pass
        except:
        ...
        try:
            ...
            #----rating list
        try:
            rating_list = bs.find('div', {'class': 'com-rating__list'}).find_all('div', {'class': 'com-rating__list-element'})
            for rate in rating_list:
                if rate.find('span').text.lower() == 'team':
                    data[DataKeys.TEAM_SCORE] = re.findall('\d{1,3}\%', rate.find('div', {'class': 'progress-bar'}).find('span')['style'])[0]
        except:
        ...
        try:
            ...
            for page in social_pages:
                try:
                    ...
                    self.logger.error('Unsupported Community type for scrapping --> {} '.format(page.find('i')['class'][1].split('-')[1]))
                    pass
        except:
